export const avatars = [
  "https://api.dicebear.com/6.x/adventurer/svg?seed=Lucy",
  "https://api.dicebear.com/6.x/adventurer/svg?seed=Casper",
  "https://api.dicebear.com/6.x/adventurer/svg?seed=Garfield",
  "https://api.dicebear.com/6.x/adventurer/svg?seed=Misty",
  "https://api.dicebear.com/6.x/adventurer/svg?seed=Mia",
  "https://api.dicebear.com/6.x/adventurer/svg?seed=Missy",
];
